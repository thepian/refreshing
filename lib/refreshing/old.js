...
*/
module.exports = {
	"watchTrees": function(t1,t2,t3) {

		  function onChange (f, curr, prev) {
...
function watchTree( root, options, watchCallback, initCallback ) {
  if (!watchCallback) {watchCallback = options; options = {}}

  walk(root, options, function (err, files) {
    if (err) throw err;
    console.log("Watching files",files,err);
...
function compileCSS(file,dest){
    ...
    var command = "lessc "+file.replace(/\s+/g,"\\ ")+" "+dest+"/"+filename.replace(/\s+/g,"\\ ")+".css";
