...
(function () {
  ...
  function folderChanged(folderPath, fnAdd, fnRemove, fnChange, fnWatched) 
  {
    return function (event, filename) {
      ...
      fs.stat(folderPath, function(err) {
        ...
        if (reset) {
          handles[folderPath] = fs.watch(folderPath, folderChanged(folderPath, fnAdd, fnRemove, fnChange, fnWatched));
        }
          ...
          files.forEach(function _forEach(file){
            ...
            fs.stat(fPath, function _stat(err, stats) {
              ...
              if (stats.isFile()) {
                ...
                watchFolderTree(fPath, fnAdd, fnRemove, fnChange, Infinity, fnWatched);     //if we have a dir, match sure it is watched
              }
            });
          });
        });
  ...
  //directories
  function watchFolderTree(fPath, fnAdd, fnRemove, fnChange, depth, fnWatched) 
  {
    ++fnWatched.nesting;
        ...
        watcher.checkFile(fPath, options.strict, function _checkFile(err, result) {
          ...
          if (!result) {
            watcher.addFile(fPath, function _addFile() {
              ...
              handles[fPath] = fs.watch(fPath, folderChanged(fPath, fnAdd, fnRemove, fnChange, fnWatched));
            });
          }
        });
...
//            console.log('depth later ' + depth)
            watchFolderTree(rPath, fnAdd, fnRemove, fnChange, depth - 1, fnWatched);
          });

        --fnWatched.nesting;
//        console.log("done watching down to",fnWatched.nesting);
        if (fnWatched.nesting == 0) fnWatched();
        });
      }
        ...
        --fnWatched.nesting;
//        console.log("done watching down to",fnWatched.nesting);
        if (fnWatched.nesting == 0) setTimeout(fnWatched,0);
        });
      }
    });
  }
  ...
  st.watch =  function(reqPath, opts, fnAdd, fnRemove, fnChange, fnWatched) {
    fnWatched.nesting = 0;
    ...
    fs.exists(nPath, function _exists(exists) {
      ...
      watchFolderTree(nPath, fnAdd, fnRemove, fnChange, depth, fnWatched);
    });
  };
}());
