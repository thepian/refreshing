...
SourceTree.prototype._scanExisting = function(p,stat) {
	...
	for(var j=0,n,last=sfn.length-1; n=sfn[j]; ++j) {
		...
		else if (_config) child.applyConfig(_config);// child.config = _config;
...
SourceNode.prototype.applyConfig = function(config) {
	this.config = config;
	this.config.skip = config.toBeIgnored(this);
};
...
SiteConfig.prototype.toBeIgnored = function(node) {
	...
	var skip = this.underscore || this.leadDot;
	if (this.lessPath && this.lessPath == node.pathPrefix().substring(0,this.lessPath.length)) skip = false;
	if (this.scssPath && this.scssPath == node.pathPrefix().substring(0,this.scssPath.length)) skip = false;
	if (this.libsPath && this.libsPath == node.pathPrefix().substring(0,this.libsPath.length)) skip = false;
	if (this.partsPath && this.partsPath == node.pathPrefix().substring(0,this.partsPath.length)) skip = false;
	return skip;
};
