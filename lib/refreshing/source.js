...
SourceTree.prototype.trigger = function(ev) {
	...
	if (ev.isDelete()) {
		...
		var _children = this.root.children;
		for(var j=0,n,last=sfn.length-1; n=sfn[j]; ++j) {
			child = _children[n];
			_children = child.children;
		}
      	...
      	try{
        	...
        	//console.log("state for",ev.name,stat);
      	} catch (e) {
        	ex = e;
      	}
      	if (child.isChanged(stat)) {
...
function SourceNode(tree,name,parent,leaf,stat) {
	...
	if (name) {
		this.name = name;
		this.underscore = (name[0] == "_") || parent.underscore;
...
SourceNode.prototype.generateLess = function() {
	...
	parser.parse(src,function(err,tree) {
		if (err) {
			...
			//console.log("tree imports",tree.imports || "no imports");
			var css = tree.toCSS({
