...
SourceNode.prototype.loadedConfig = function() {
	...
	if (this.tree.enableServer && this.config.serverPort && outputPath) {
		console.log("Server on",this.config.serverPort,"for",outputPath);
		var connect = require('connect');

		var app = connect()
			.use(connect.static(outputPath))
...
SourceNode.prototype.updateHtml = function(action) {
	switch(action) {
		case "init":
		case "added":
		case "changed":
...
SourceNode.prototype.generateHtml = function() {
	//console.log("inPath",this.tree.rootPrefix,"." + this.childPath().join(path.sep));
	var inPath = path.resolve(this.tree.rootPrefix,"." + this.childPath().join(path.sep));
	for(var n in this.config.outputs) {
		var outPath = this.config.outputs[n].resolve(this.childPath().join(path.sep));
...
SourceNode.prototype["ext .html"] = function() {
	if (! this.underscore) {
		this._update = this.updateHtml;
		this.generate = this.generateHtml;
	}
};
...
// sound
SourceNode.prototype["ext .mp3"] =
SourceNode.prototype["ext .wav"] =
SourceNode.prototype["ext .m4a"] = function() {
	if (! this.underscore) {
		this.update = this.updateCopy;
		this.generate = this.generateCopy;
	}
