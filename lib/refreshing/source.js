...
SourceTree.prototype.FILETYPES = {
	"_config.yml": function(child) {
		child.update = child.updateConfig;
	},

	".less": function(child) {

		child.update = child.updateLess;
		child.generate = child.generateLess;
	},

	".scss": function(child) {
		child.update = child.updateScss;
		child.generate = child.generateScss;
	}

};

SourceTree.prototype.addChild = function(children,name,parent,leaf,stat,type) {
	var child = children[name] = new SourceNode(this,name,parent,leaf,stat);
...
SourceTree.prototype.ensureChild = function(children,name,parent,leaf,stat) {
	if (children[name] == undefined) {
		...
		if (!this.starting && leaf) console.log("Changed:",children[name].childPath().join("/"));
	}
...
SourceTree.prototype.removeChild = function(children,name,parent,leaf) {
	if (leaf) {
		...
		if (!this.starting) console.log("Removed:",children[name].childPath().join("/"),"config:",children[name].config.rootPath);
...
SourceNode.prototype.updateConfig = function(action) {
	if (this.parent.config == undefined) {
		...
		this.parent.config = new SiteConfig(this.tree.rootPath,this.parent.childPath(),"_config.yml");
	}
};
